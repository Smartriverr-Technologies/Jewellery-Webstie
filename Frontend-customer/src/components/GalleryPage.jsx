...
import axios from 'axios';
import {
  Container, Typography, ImageList, ImageListItem, CircularProgress,
  Alert, Box, Fade, Dialog, IconButton, Button, TextField
} from '@mui/material';
import CloseIcon from '@mui/icons-material/Close';
import { styled } from '@mui/material/styles';
...
const GalleryPage = () => {
  ...
  // ✅ Mutation to add new gallery image
  const addImageMutation = useMutation({
    mutationFn: async (newImage) => {
      const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
      return await api.post('/api/gallery', newImage, config);
    },
    onSuccess: () => {
  ...
  const handleAddImage = (e) => {
    ...
    addImageMutation.mutate({ imageUrl, altText });
  };
      ...
      {userInfo?.isAdmin && (
        <Fade in timeout={800}>
          <Box sx={{ mb: 6, textAlign: 'center' }}>
            <Typography variant="h5" gutterBottom>Add Image to Gallery</Typography>
            <Box
              component="form"
            ...
            >
              ...
              >
                {addImageMutation.isLoading ? 'Adding...' : 'Add to Gallery'}
              </Button>
            </Box>
          </Box>
        </Fade>
